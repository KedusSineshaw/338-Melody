...
export default function FileUploader({ onUploaded, onBack }) {
  ...
  async function handleUpload() {
    ...
    try {
        ...
        xhr.onreadystatechange = () => {
          if (xhr.readyState === 4) {
            if (xhr.status >= 200 && xhr.status < 300) {
              ...
              reject(
                new Error(`AI or Not upload failed with status ${xhr.status}`)
              );
            }
          }
        };
      ...
      if (!aiJson.job_id) {
        throw new Error("AI or Not did not return a job_id");
      }
      const aiJobId = aiJson.job_id;

      // 2) CYANITE
      const cyForm = new FormData();
      cyForm.append("file", file);

      const cyRes = await fetch("http://localhost:5000/api/cyanite/analyze", {
        method: "POST",
        body: cyForm,
      });

      if (!cyRes.ok) {
        throw new Error("Cyanite analysis request failed");
      }
      ...
      irForm.append("file", file);

      const irRes = await fetch("http://localhost:5000/api/ircam/analyze", {
        method: "POST",
        body: irForm,
      });

      if (!irRes.ok) {
        throw new Error("IRCAM analysis request failed");
      }

      const irJson = await irRes.json();
      const ircamJobId = irJson.jobId;

      onUploaded(aiJobId, cyaniteId, ircamJobId);
    } catch (err) {
      console.error(err);
